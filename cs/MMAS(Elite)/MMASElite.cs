...
class MMASElite{
    ...
    public MMASElite(int cityNum, string filename, double initPheromone, double alpha, double beta, double rho, double pheromoneQ)
    {
    ...
    public void generateRoute(Agent agent){
        ...
        if(bestFitness == -1 || distance < bestFitness){
            ...
            bestAgent = agent;
            bestRoute = new List<int>(agent.route);
        }
    ...
    public void updatePheromone(){
        double add = pheromoneQ * (eliteAntNum+1) / bestFitness;        // eliteAntNumに1を足さないと下限が0になり、経路が生成できない
        double newPheromone;
